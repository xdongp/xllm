...
graph LR
    A[准备输入] --> B[查询KV缓存]
    B --> C[准备past_key_values]
    C --> D[执行模型推理]
    D --> E[提取past_key_values]
    E --> F[更新KV缓存]
    F --> G[返回结果]
...
| 优化技术 | 推理时间（ms/token） | 加速比 | 内存占用 |
|---------|-------------------|--------|---------|
| 无优化 | 50 | 1x | 100% |
| KV缓存 | 15 | 3.3x | 120% |
| KV缓存 + INT8 | 8 | 6.25x | 30% |
| KV缓存 + INT8 + torch.compile | 6 | 8.3x | 30% |
| 批处理（8个请求） | 15/8=1.875 | 26.7x | 30% |

### 6.2 不同模型性能

| 模型 | 参数量 | 推理时间（ms/token） | 吞吐量（tokens/s） |
|------|-------|-------------------|-------------------|
| Qwen-0.6B | 0.6B | 5 | 200 |
| Qwen-7B | 7B | 15 | 66 |
| Qwen-14B | 14B | 30 | 33 |
| Qwen-72B | 72B | 150 | 6.7 |
